...
import { Play, RotateCcw, AlertTriangle, Terminal, X } from 'lucide-react'
  ...
  return (
    <div className={`fixed bottom-0 left-0 right-0 z-[1000] transition-all duration-300 ${isOpen ? 'translate-y-0' : 'translate-y-[calc(100%-8px)]'}`}>
      ...
      <div 
        ...
        className="h-2 bg-gov-orange cursor-pointer hover:h-3 transition-all flex items-center justify-center group"
      >
      ...
      <div className="bg-slate-900 text-white p-6 shadow-2xl border-t border-white/10">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
             <Terminal className="text-gov-orange w-6 h-6" />
             <h3 className="text-lg font-mono font-bold tracking-widest text-gov-orange">DEMO_EXECUTIVE_CONTROL</h3>
             <span className="text-[10px] bg-gov-orange/20 text-gov-orange px-2 py-0.5 rounded border border-gov-orange/30">V.2.0</span>
          </div>
        ...
        <div className="grid grid-cols-4 gap-6">
          <div className="space-y-3 bg-white/5 p-4 rounded-lg border border-white/5">
             ...
             <button 
               onClick={handleReset}
               className="w-full flex items-center justify-center gap-2 bg-gov-red hover:bg-gov-red/80 text-white py-3 rounded font-bold text-sm transition-all"
             >
          ...
          <div className="col-span-3 space-y-3 bg-white/5 p-4 rounded-lg border border-white/5">
             ...
             <div className="grid grid-cols-3 gap-3">
                <button onClick={() => triggerScenario(1)} className="flex items-center justify-between bg-white/10 hover:bg-white/20 p-3 rounded text-left group">
                  <div>
                    <p className="text-xs font-bold text-gov-orange">S1: LOW RISK</p>
                ...
                <button onClick={() => triggerScenario(2)} className="flex items-center justify-between bg-white/10 hover:bg-white/20 p-3 rounded text-left group">
                  <div>
                    <p className="text-xs font-bold text-gov-amber">S2: MEDIUM RISK</p>
                ...
                <button onClick={() => triggerScenario(3)} className="flex items-center justify-between bg-white/10 hover:bg-white/20 p-3 rounded text-left group">
                  <div>
                    <p className="text-xs font-bold text-gov-red">S3: CRITICAL</p>
